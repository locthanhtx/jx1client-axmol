...
KSelfBreathLight::KSelfBreathLight()
{
	...
	dwLastBreathTime = m_pStartUpdate->tv_usec/1000;//timeGetTime();
}
	...
	if (pLeaf->eLeafType == KIpotLeaf::IPOTL_T_RUNTIME_OBJ)
	{
		...
		if (pRtoLeaf->eLayerParam & IPOT_RL_LIGHT_PROP)
		{
			...
			if(pLight)
			{
				...
				CoreGetGameObjLightInfo(pRtoLeaf->uGenre, pRtoLeaf->nId, &lightInfo); //�滭����
				...
				case CGOG_OBJECT:
					...
					break;
				case CGOG_MISSLE:
					///pLight->m_nRadius = 0;//MAIN_PLAYER_LIGHT_RADIUS;
...
//	}
    ...
    //pLightingArray[0] = m_dwAmbient;    // ���û�����
}
...
void KIpoTree::AddBuildinLight(KBuildInLightInfo* pLights, int nCount)
{
	...
	for(int i=0; i<nCount; i++)
	{
		...
		if(pLight)
		{
			...
			pLight->fCycle = 100 * 0.875f + (int)g_Random(60 / 4);
			m_LightList.push_back(pLight);
		}
	}
}
