...
bool KgameWorld::init()
{
	...
	*/
	...
	imidspr_1->setPosition(ax::Vec2(visibleSize.width-nItEmBoxSize_1.width*0.5-10,visibleSize.height/2+20));   //重左下角开始移动
	ParentNode_item->addChild(imidspr_1,1,1);
	//456
	Sprite *imidspr_2= Sprite::create("ui/state/kbgitem.png");
	imidspr_2->setPosition(ax::Vec2(visibleSize.width-nItEmBoxSize_1.width*1.5-10,visibleSize.height/2+20)); //重左下角开始移动 +nItEmBoxSize_1.height/2
	ParentNode_item->addChild(imidspr_2,1,2);
	//789
	Sprite *imidspr_3= Sprite::create("ui/state/kbgitem.png");
	imidspr_3->setPosition(ax::Vec2(visibleSize.width-nItEmBoxSize_1.width*2.5-10,visibleSize.height/2+20));   //重左下角开始移动+nItEmBoxSize_1.height/2
	...
	pRoleeqItem = MenuItemImage::create( "ui/tools/role.png", "ui/tools/role_d.png", "ui/tools/role_s.png",CC_CALLBACK_1(KgameWorld::mRoleStatusCallback, this));
	ppItemsItem = MenuItemImage::create( "ui/tools/itembox.png", "ui/tools/itembox_d.png", "ui/tools/itembox_s.png",CC_CALLBACK_1(KgameWorld::mItemsCallback, this));
	pItemExItem = MenuItemImage::create( "ui/tools/friend.png", "ui/tools/friend_d.png", "ui/tools/friend_s.png",CC_CALLBACK_1(KgameWorld::mItemExCallback, this));
	pSkillsItem = MenuItemImage::create( "ui/tools/skill.png", "ui/tools/skill_d.png", "ui/tools/skill_s.png",CC_CALLBACK_1(KgameWorld::mSkillsCallback, this));
	pTeamItem = MenuItemImage::create( "ui/tools/team.png", "ui/tools/team_d.png", "ui/tools/team_s.png",CC_CALLBACK_1(KgameWorld::mTeamCallback, this));
	pFactionItem = MenuItemImage::create( "ui/tools/tong.png", "ui/tools/tong_d.png", "ui/tools/tong_d.png",CC_CALLBACK_1(KgameWorld::mFactionCallback, this));
	pOptionsItem = MenuItemImage::create( "ui/tools/option.png", "ui/tools/option_d.png", "ui/tools/option_d.png",CC_CALLBACK_1(KgameWorld::mOptionsCallback, this));
	ptaskItem = MenuItemImage::create( "ui/tools/task.png", "ui/tools/task_d.png", "ui/tools/task_d.png",CC_CALLBACK_1(KgameWorld::mTaskCallback, this));
	...
	if (pRoleeqItem)
	{
		pControlMenu = Menu::create(pRoleeqItem,ppItemsItem,pItemExItem,pSkillsItem,ptaskItem,pTeamItem,pFactionItem,pOptionsItem,pPaiItem,pQizItem,NULL);
		...
		*/
		...
		pControlMenu->setPosition(ax::Vec2(visibleSize.width/2+origin.x+80,visibleSize.height-40));//ax::Vec2::ZERO
		this->addChild(pControlMenu,ORDER_UI_DIALOD);
	}
	//主技能按钮
	pMainSkillItem = MenuItemImage::create(
		"ui/at/mr-1.png",
		"ui/at/mr-2.png",
        CC_CALLBACK_1(KgameWorld::mianSkillCallback, this)); //回调函数

	pMainSkillItem->setScale(0.7);
	pMainSkillItem->setPosition(ax::Vec2(origin.x + visibleSize.width - pMainSkillItem->getContentSize().width/2-40 , //20
		origin.y + pMainSkillItem->getContentSize().height/2+30));//10

	//攻击模式却换
	//主技能按钮
	MenuItemImage *pUseModelItem = MenuItemImage::create(
	...
	if (pHorse)
	{
		...
		pHorseItem->setPosition(ax::Vec2(origin.x +BTNOFFSETX ,
	...
	if (pSit)
	{
		...
		pSitItem->setPosition(ax::Vec2(origin.x +BTNOFFSETX ,
	...
	pPickItem = MenuItemImage::create(
		"ui/btn/pick_01.png",
		"ui/btn/pick_02.png",
		"ui/btn/pick_02.png",CC_CALLBACK_1(KgameWorld::pickCallback, this));
	pPickItem->setScale(2,2);
	pPickItem->setPosition(ax::Vec2(origin.x +BTNOFFSETX+50,
		origin.y + pSitItem->getContentSize().height/2+60+BTNOFFSETY));
	...
	{//打坐按钮
		...
		pRunItem->setPosition(ax::Vec2(origin.x + BTNOFFSETX,
	...
	if (pPk)
	{
		...
		pPkItem->setPosition(ax::Vec2(origin.x +BTNOFFSETX ,
	...
	{//交易按钮
		...
		pExchangeItem->setPosition(ax::Vec2(origin.x +BTNOFFSETX ,
	...
	{//简化地图
		...
		//origin.x + visibleSize.width - pExchangeItem->getContentSize().width/2-110
		pDelmapItem->setPosition(ax::Vec2(origin.x +BTNOFFSETX ,
	...
	//创建一个按钮菜单
	pcontrolMenu = Menu::create(pUseModelItem,pHorseItem,pSitItem,pRunItem,pPkItem,pExchangeItem,pquxiaItem,pyincItem,pDelmapItem,playerfunItem,playersellItem,pPickItem,qianghItem,pMainSkillItem,NULL);
	...
	//渲染纹理 图片数据
	/*int nFrams=0;
	Texture2D *Cur = _getinidata.getinidata_new("\\spr\\skill\\1502\\kl\\kl_150_jiankun_fu.spr",10,&m_nWidth,&m_nHeight,&nFrams);
	testSprite = NULL;
	if (Cur)
	{
			...
			if (testSprite)
			   ...
			   testSprite->runAction(Sequence::create(animate,CallFunc::create(this, callfunc_selector(KgameWorld::finish)),NULL));
   ...
   /* for (int i=0;i<MAX_FUZHUSKILL_COUNT;i++)
	{
		...
		skillstick[i].skillstick=KSkillRocker::KSkillRockerWithCenter(ax::Vec2(visibleSize.width,-100.0f),60.0f ,skillstick[i].controlSprite_s ,skillstick[i].ygSprite_s,false);
	...
	/*skillstick=KSkillRocker::KSkillRockerWithCenter(ax::Vec2(visibleSize.width,-100.0f),60.0f ,controlSprite_s ,ygSprite_s,false);
...
//	ui::Layout *MainChatSprite = ui::Layout::create();
    ...
    auto callback = [this](ax::Node* sender) {
        KgameWorld::closeChatlistPad(sender);
    };
	...
	chatSendLabel->setPosition(ax::Vec2(chatsendSpr->getContentSize().width/2-chatSendLabel->getContentSize().width/4,chatsendSpr->getContentSize().height/4));
	...
	//plifeLabel = Label::createWithTTF("0/0",UI_GAME_FONT_DEFAULT_VN,24,nRoleStateLifeNode->getContentSize(),TextHAlignment::CENTER);//Arial
...
//主角当前坐标
	...
	ptestLabel->setPosition(ax::Vec2(visibleSize.width-nMapSize.width/2,visibleSize.height-nMapSize.height+20));
...
//	SPRFRAMSINFO nSprInfo;
	...
	MenuItemSprite * maxMapConfirm = MenuItemSprite::create(sprite_max_normal, sprite_max_select, CC_CALLBACK_1(KgameWorld::maxmapCallBackFunc, this));
	...
	MenuItemSprite *pHuoDongSpr=MenuItemSprite::create(btn_normal_huo_1,btn_select_huo_1,NULL,CC_CALLBACK_1(KgameWorld::huodongCallback, this));
	...
	MenuItemSprite *pAutoSpr=MenuItemSprite::create(btn_normal_Auto_1,btn_select_Auto_1,NULL,CC_CALLBACK_1(KgameWorld::autoCallback, this));
	...
	MenuItemSprite *pCaveSpr=MenuItemSprite::create(btn_normal_Cave_1,btn_select_Cave_1,NULL,CC_CALLBACK_1(KgameWorld::caveCallback, this));
...
//	this->addChild(red,10,OBJ_NODE_COUNT);
    ...
    auto callback1 = [this](ax::Node* sender) {
        KgameWorld::closeplayerfunPadCallback(sender);
    };
...
void KgameWorld::mQizCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeDyshopPadCallback(sender);
    };
...
void KgameWorld::mTaskCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closetaskPadCallback(sender);
    };
...
void KgameWorld::mOptionsCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeescPadCallback(sender);
    };
...
void KgameWorld::mFactionCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeTongPadCallback(sender);
    };
...
void KgameWorld::mTeamCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeTeamPadCallback(sender);
    };
...
{//OBJ_NODE_SKILL
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeSkillPadCallback(sender);
    };
...
void KgameWorld::mItemExCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeItemExPadCallback(sender);
    };
...
void KgameWorld::mItemsCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeItemPadCallback(sender);
    };
		...
		case 0://tự động
			{
                ...
                auto nCurMenuItem = MenuItemSprite::create(Sprite::createWithSpriteFrame(nTempFrame), Sprite::createWithSpriteFrame(nTempFrame), nullptr);
...
void KgameWorld::OnDirectSendChannelMessage(DWORD nChannelID, BYTE cost, const char* Buffer, int nLen,int nItemDwIdx)				//发送频道聊天到服务器
{
	...
	{
		uint32_t chatsize = sizeof(CHAT_CHANNELCHAT_CMD) + nLen;
		uint32_t pckgsize = sizeof(tagExtendProtoHeader) + chatsize;
...
void KgameWorld::OnSendSomeoneMessage(const char* Name, const char* Buffer, int nLen,BOOL nIsItemLink)	//发送someone聊天到服务器
{
	...
	{
		uint32_t chatsize = sizeof(CHAT_SOMEONECHAT_CMD) + nLen;
		uint32_t pckgsize = sizeof(tagExtendProtoHeader) + chatsize;
...
void KgameWorld::ReplaceSpecialName(char* szDest, uint32_t nDestSize, char* szSrc)
{
	if  (sfunLayer)
...
void KgameWorld::playerSellCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closesellItemPadCallback(sender);
    };
...
void KgameWorld::playerfunCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeplayerfunPadCallback(sender);
    };
...
void KgameWorld::huodongCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node *sender) {
        this->closemapPadCallback(sender);
    };
...
void KgameWorld::mianSortSillCallback(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node *sender) {
        this->closesortskillPad_r(sender);
    };
...
void KgameWorld::setchatCurChannel(char  *name)
{
	...
	*/
	...
	{//开始变更纹理
		// Texture2D * bgCur = NULL;
		// Sprite* btn_normal_sprite = NULL;
		// Sprite* btn_select_sprite = NULL;
		// char nSprName[128]={0};
		// int m_nWidth,m_nHeight,nFrams;
		// SPRFRAMSINFO nSprInfo;
		// ZeroMemory(&nSprInfo,sizeof(nSprInfo));
		...
		// bgCur = _getinidata.getinidata_one(nSprName,0,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
		// if (!bgCur) return;
		// 	btn_normal_sprite =Sprite::createWithTexture(bgCur);
		// ChannelSpr->setNormalImage(btn_normal_sprite);
		// bgCur = _getinidata.getinidata_one(nSprName,1,&m_nWidth,&m_nHeight,&nFrams,&nSprInfo);
		// if (!bgCur) return;
		// 	btn_select_sprite=Sprite::createWithTexture(bgCur);
		// ChannelSpr->setSelectedImage(btn_select_sprite);
		Label *tempLabel = Label::createWithTTF("",UI_GAME_FONT_DEFAULT_VN_BTN, 15,ChannelSpr->getContentSize(),TextHAlignment::CENTER);
		if (strstr(name,"CH_NEARBY"))
		{//附近
...
void KgameWorld::maxmapCallBackFunc(Ref* pSender)
{
    ...
    auto callback = [this](ax::Node *sender) {
        this->closeMaxMapPadFunc(sender);
    };
				...
				for(int i = 0 ;i<nCount;++i,++pTemp)
				{
					...
					{//如果存在就更新节点spr
						...
						{//修改精灵的纹理
							...
							RenderTexture *npicCanvas = RenderTexture::create(CCpicRect.size.width,CCpicRect.size.height,kTexture2DPixelFormat_RGBA8888,0);
	...
	{//同一个区域的
		...
		for(int i = 0;i<nPos;++i/*,++pTemp*/)  //
		{
			...
			if (bgCur)
			{
				...
				CCRectZero = ax::Rect(pTemp[i].oImgLTPos.nX,pTemp[i].oImgLTPos.nY,pTemp[i].oImgRBPos.nX,pTemp[i].oImgRBPos.nY);
			...
			{//Nếu không có kết cấu, hãy tô nó bằng một khối màu vàng
				...
				if (pTemp[i].oImgRBPos.nY > CCRectZero.size.height)
					pTemp[i].oImgRBPos.nY = CCRectZero.size.height;
				CCRectZero = ax::Rect(pTemp[i].oImgLTPos.nX,pTemp[i].oImgLTPos.nY,pTemp->oImgRBPos.nX,pTemp->oImgRBPos.nY);
...
void KgameWorld::DrawPrimitives_obj(int nObjIndex,int nPos, KRUImage* pPrimitives, unsigned int uGenre, int bpartNo,int nOrDer,int bSinglePlaneCoord,ax::Color3B nObjColor,char *nObjName,char *_clientObjName)
{//地上的物品
		...
		if (nObjName)
		{
			...
			if (!nPartlabel)
			{
				...
				if  (_clientlanguage!=1)
				   ...
				   nTempSize.height = 14;
				nPartlabel = Label::createWithTTF("",UI_GAME_FONT_DEFAULT_VN,13,nTempSize,TextHAlignment::CENTER);//Arial
...
Texture2D * KgameWorld::ccgetTxtData_g(char *sprPath,int nFramIndex,int nMpsX,int nMpsY,int nMpsZ,int *mScreeX,int *mScreeY,int bRenderFlag,int bSinglePlaneCoord,int *mHeadw,int *mHeadh)
{
	    if  (sprPath==NULL)
			return NULL;
...
Texture2D * KgameWorld::ccgetTxtData(char *sprPath,int nFramIndex,int nMpsX,int nMpsY,int nMpsZ,int *mScreeX,int *mScreeY,int bRenderFlag,int bSinglePlaneCoord,int *mHeadw,int *mHeadh)
{
	    if  (sprPath==NULL)
			return NULL;
...
void KgameWorld::DrawPrimitives_NpcOther(int nNpcIndex,int m_Kind,bool isClear)
{
	...
	{//Bắt đầu vẽ thanh trạng thái tên NPC_NAME
		...
		if (!objnamelabel)
		{
			...
			//objnamelabel = Label::createWithTTF("",UI_GAME_FONT_DEFAULT,nFontSize,nTempSize,TextHAlignment::CENTER);//Arial
			...
			{//
				...
				{//如果是摆摊的  PLAYER_TAN
					if (!itemTanlabel)
					{
						...
						// stroke only label
						//objnamelabel = Label::createWithTTF("",UI_GAME_FONT_DEFAULT,nFontSize,nTempSize,TextHAlignment::CENTER);//Arial
						itemTanlabel = Label::createWithTTF("",UI_GAME_FONT_DEFAULT_VN,50,nTempSize,TextHAlignment::CENTER);//Arial
			   ...
			   if  (nIsShowTitle)
			   {
				   ...
				   {//帮派 显示帮派称号
					  ...
					  // t_sprintf(nTempTongName,Npc[nNpcIndex].TongName);
					   ...
					   if (!ToneNameLabel)
					   {
							...
							nDsize.height = 0;
						   //ToneNameLabel = Label::createWithFontDefinition(UTEXT(nTempTongName,1).c_str(),strokeTextDef);
						   // nTitleLabel =  Label::createWithTTF(UTEXT(PlayerRank,1).c_str(),UI_GAME_FONT_DEFAULT,12);//Arial
						   ToneNameLabel = Label::createWithTTF(nTempTongName,UI_GAME_FONT_DEFAULT_VN, 13,nDsize,TextHAlignment::CENTER);
				   ...
				   {//spr称号
					   ...
					   nPath="\\spr\\Ui4\\KgameWorld\\btn_BatPk.spr";  //红心右旋转
				   ...
				   if (Npc[nNpcIndex].nReBorn>0)
				   {
					   ...
					   {
                           auto callback = [this](ax::Node *sender) {
                               this->closeShowSpritePad(sender);
                           };

						   nPath="\\spr\\Ui4\\KgameWorld\\btn_BatPk.spr"; //凤凰印
						   ReBornLabel  = KuiShowSprite::create(nPath,0,nTempHeight+20,this, callback,true,RUIMAGE_RENDER_FLAG_REF_SPOT);
					   ...
					   ((KuiShowSprite*)ReBornLabel)->setShowPosition(0-nTempSize.width/2,nTempHeight+20);  //ax::Vec2(0,nOffHeight+20)
					   //((KuiShowSprite*)ReBornLabel)->setRegisterWithTouchDispatcher(128);
				   }
				   else
				   {
				   ...
				   {//新文字称号
					   ...
					   //nTitleLabel =  Label::createWithTTF(UTEXT(PlayerRank,1).c_str(),UI_GAME_FONT_DEFAULT,12,nSize,TextHAlignment::CENTER);//Arial
					   if (!nTitleLabel)
					   {
						   //nTitleLabel = Label::createWithFontDefinition(UTEXT(PlayerRank,1).c_str(),strokeTextDef);
						   nTitleLabel = Label::createWithTTF(PlayerRank,UI_GAME_FONT_DEFAULT_VN, 13,nDsize,TextHAlignment::CENTER);
			   ...
			   if  (m_Kind==kind_player && nNpcIndex == Player[CLIENT_PLAYER_INDEX].m_nIndex)
			   {
				...
				{//Trong chiến đấu tự động Khi treo
					if (!objautoplaylabel)
					{
						...
						nPath="\\spr\\Ui4\\KgameWorld\\btn_BatPk.spr";//"\\spr\\skill\\others\\亢龙有悔.spr";//
						objautoplaylabel  = KuiShowSprite::create(nPath,0,nTempHeight+2,this, callback,true,RUIMAGE_RENDER_FLAG_REF_SPOT);
				...
				else
				{
				 ...
				 {//自动战斗中 挂机中
					if (objautoplaylabel)
					{
					   ...
					   ((KuiShowSprite*)objautoplaylabel)->setShowPosition(0-nTempSize.width/2,nTempHeight+2);  //ax::Vec2(0,nOffHeight+20)
					}
				 }
			   }
			   ...
			   npcLifespr->setPosition(ax::Vec2(-nNpcOtherNode->getContentSize().width-npcLifespr->getContentSize().width/2,nOffHeight));
			   if (relation_enemy == NpcSet.GetRelation(nNpcIndex,Player[CLIENT_PLAYER_INDEX].m_nIndex) &&
		...
		case kind_normal:
			{//ten quai vat
				...
				if (npcSexspr)
					npcSexspr->setPosition(ax::Vec2(0-npcSexspr->getContentSize().width/2-objnamelabel->getContentSize().width/2+30,nNpcoffY+8));
...
{
	if  (!nNpcOtherNode) return NULL;//ax::Size(0,0);
			...
			{//默认位置在节点的中间
				...
				case 7:
					{//右手
						...
						if (bgCur)
						{//修改精灵的纹理
							Sprite * nPartSpr = (Sprite *)(nPartCurNode->getChildByTag(UI_EQ_PART_RIGTHHD));
							if (!nPartSpr)
								break;
				...
				case 16:
					{
						...
						if (bgCur)
						{//修改精灵的纹理
							Sprite * nPartSpr = (Sprite *)(nPartCurNode->getChildByTag(UI_EQ_PART_PIFENG));
							if (!nPartSpr)
								break;
							if (Npc[nNpcIndex].m_PifengType<=0)
								nPartSpr->setVisible(false);//设置不可见
		 ...
		 for (i=0;i<MAX_FUZHUSKILL_COUNT;i++)
		 {
			...
			{//有空位置
				...
				{//修改精灵的纹理
					...
					//static MenuItemSprite * create(Node* normalSprite, Node* selectedSprite, Node* disabledSprite, Ref* target, SEL_MenuHandler selector)
					auxiliaryskill[i].auxiliaryskillSpr=MenuItemSprite::create(btn_normal_sprite,btn_select_sprite,NULL,CC_CALLBACK_1(KgameWorld::auxiliaryskillCallback, this));
					...
					auxiliaryskill[i].auxiliaryskillSpr->setPosition(ax::Vec2(auxiliaryskill[i].OffsetX,auxiliaryskill[i].OffsetY));
	...
	case SMT_CLIQUE://帮会
		{
			...
			case SMCT_UI_TONG_JOIN_APPLY:
				{//通知有人申请入帮派
                    ...
                    auto callback = [this](ax::Node* sender) {
                        this->closeNewDialogPadCallback(sender);
                    };
					Layer * sDialogLayer_1 = KuiTalkDialog1::create(pMsg->szMessage,"Đồng ý tham gia","Từ chối tham gia",SMCT_UI_TONG_JOIN_APPLY,this,callback,nTempTong->nIndex,nTempTong->Name);
		...
		{//队伍
			...
			case SMCT_UI_TEAM_APPLY:
				{//答应或拒绝加入队伍的申请,
					...
					Layer * sDialogLayer_1 = KuiTalkDialog1::create(pMsg->szMessage,"Đồng ý tham gia","Từ chối tham gia",SMCT_UI_TEAM_APPLY,this,callback,nTarNpcDwid);
					this->addChild(sDialogLayer_1,ORDER_UI_DIALOD,OBJ_NODE_TALKDIALOG_NEW);
				}
				break;
			case SMCT_UI_TEAM_INVITE:
				{//邀请入队的应答
					...
					Layer * sDialogLayer_1 = KuiTalkDialog1::create(pMsg->szMessage,"Đồng ý tham gia","Từ chối tham gia",SMCT_UI_TEAM_INVITE,this,callback,nTarPlaerindex);
					this->addChild(sDialogLayer_1,ORDER_UI_DIALOD,OBJ_NODE_TALKDIALOG_NEW);
				}
			break;
			default:
	...
	case SMT_SYSTEM:
		{
			...
			case SMCT_UI_TRADE:
				{//交易
					...
					Layer * sDialogLayer_1 = KuiTalkDialog1::create(pMsg->szMessage,"Đồng ý giao dịch","Từ chối giao dịch",SMCT_UI_TRADE,this,callback,nTarPlaerindex);
					this->addChild(sDialogLayer_1,ORDER_UI_DIALOD,OBJ_NODE_TALKDIALOG_NEW);
				}
				break;
			default:
...
#define	MSG_RENASCENCE	       "你已经身受重伤!"
					...
					Layer * sDialogLayer_1 = KuiTalkDialog1::create(MSG_RENASCENCE,"Về thành dưỡng sức","Hồi sinh tại chỗ",SMCT_UI_RENASCENCE,this,callback);
					m_talkDialogPadIsOpen_1 = true;
					this->addChild(sDialogLayer_1,ORDER_UI_DIALOD,OBJ_NODE_TALKDIALOG_NEW);
				}
				break;
...
void KgameWorld::CreatTalkDialog(KUiInformationParam* pWordDataList, int nCount)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeTaikDialogPadCallback(sender);
    };
...
{//
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeNpcDialogPadCallback(sender);
    };
...
void  KgameWorld::openstringBox(const char* pszTitle,int nMinLen, int nMaxLen,const char* pszCallback,const char* pszError,int bNum)
{
	...
	Layer * getStringDialogLayer = KuigetStringInUI::create(strTitle,nMinLen,nMaxLen,pszCallback,pszError,bNum,this,callback);
...
void  KgameWorld::openNpcshop(int nshopidx)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->NpcshopPadCallback(sender);
    };
...
void  KgameWorld::OpenCreatTongSheet()
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeTongSheetPadCallback(sender);
    };
...
void  KgameWorld::OpenItenLinkDialog(KUiLianItem*  pDest)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeLinkPadCallback(sender);
    };
...
void  KgameWorld::OpenPlayerTradeDialog(KUiPlayerItem* pDest)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closetradePadCallback(sender);
    };
...
void  KgameWorld::OpenPlayerShopDialog(KUiPlayerItem* pDest)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->playershopPadCallback(sender);
    };
...
void  KgameWorld::OpenViewStateDialog(KUiPlayerItem* pDest)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeViewPadCallback(sender);
    };
...
void  KgameWorld::qiangItemCallback(Ref* pSender)
{
	...
	/*Layer * giveLayer = Kuigive::create("装备强化面板","注意:装备强化只能放一件装备和一个强化石头!友情提示:强化可升可降,需谨慎!",1,"sysqianghua","sysError",this,CC_CALLBACK_1(KgameWorld::closegivePadCallback));
...
void  KgameWorld::openGive(char* pszTitle, char* pszInitString,int nNum,char* pszCallback,char* pszError)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closegivePadCallback(sender);
    };
...
void  KgameWorld::openlaotou()
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closeLaoPadCallback(sender);
    };
...
void  KgameWorld::openProgressbar(KUiJinDuTiaoInfo* pJinDu)
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->progressPadCallback(sender);
    };
...
void  KgameWorld::openStorebox()
{
    ...
    auto callback = [this](ax::Node* sender) {
        this->closestorePadCallback(sender);
    };
...
void  KgameWorld::HoldObject_(int nType,int nBoxIndex,int nItemidx,bool isAdd)
{
	...
	{//012
		...
		if (!nIiemTempSpr)
		{
			...
			Sprite* btn_normal_sprite=Sprite::createWithTexture(bgCur);
			Sprite* btn_select_sprite=Sprite::createWithTexture(bgCur);
			nIiemTempSpr = MenuItemSprite::create(btn_normal_sprite,btn_select_sprite,NULL,CC_CALLBACK_1(KgameWorld::ImmediaCallback, this));
			nIiemTempSpr->setAnchorPoint(ax::Vec2(0,0));
			nIiemTempSpr->setPosition(ax::Vec2(0,0));
			nIiemTempSpr->setPosition(ax::Vec2(0,0));
