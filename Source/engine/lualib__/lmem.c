...
#define blocksize(b)	((uint32_t *)((char *)(b) - HEADER))

uint32_t memdebug_numblocks = 0;
uint32_t memdebug_total = 0;
uint32_t memdebug_maxmem = 0;
uint32_t memdebug_memlimit = LONG_MAX;


static void *checkblock (void *block) {
  uint32_t *b = blocksize(block);
  uint32_t size = *b;
  int i;
