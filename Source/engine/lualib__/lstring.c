...
static uint32_t hash_s (const char *s, size_t l) {
  uint32_t h = l;  /* seed */
  size_t step = (l>>5)|1;  /* if string is too int32_t, don't hash all its chars */
  for (; l>=step; l-=step)
...
void luaS_resize (lua_State *L, stringtable *tb, int newsize) {
  ...
  for (i=0; i<tb->size; i++) {
    ...
    while (p) {  /* for each node in the list */
      ...
      uint32_t h = (tb == &L->strt) ? p->u.s.hash : IntPoint(p->u.d.value);
