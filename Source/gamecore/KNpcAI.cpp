...
//	}
	...
	if (nRelation == relation_enemy) //如果是敌人关系 就攻击
	{
		...
		{//非挂机状态
			...
			/*char nMSG[64];
			sprintf(nMSG,"--当前攻击距离:--%d--生命:%d",Npc[m_nIndex].m_CurrentAttackRadius,Player[CLIENT_PLAYER_INDEX].m_Autoplay.nNpcCurlife);		 
...
void KNpcAI::MoveJiaoBen(int m_Index)
{
 ...
 if (Player[CLIENT_PLAYER_INDEX].m_Autoplay.nIsHaveDo==0)
 {
   for (int i=1;i<mRows;++i)
   {
	...
	{
	      /*char msg[64];
		  sprintf(msg,"脚本(X:%s,Y:%d)出错,(%d)停止挂机。",nDesMpsX,nDesMpsY,Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurNum);
	...
	//sprintf(msg,"提示:当前(%d)坐标,X: %d,Y:%d",Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurNum,xx,yy);
    //Player[CLIENT_PLAYER_INDEX].m_ItemList.ClientShowMsg(msg);
	...
	{//跑到了某个点
	   //sprintf(msg,"提示:走到目标,第 %d 个坐标点",Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurNum);
       //Player[CLIENT_PLAYER_INDEX].m_ItemList.ClientShowMsg(msg);
      ...
      //   MoveJiaoBen(m_Index);
	   ...
	   if (Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurNum==0)
	   {
	     ...
	     int DisJuli = g_GetDistance(xx,yy,Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurXpos,Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurYpos);
	         sprintf(msg,"X:%d,Y:%d,距离:%d,范围:%d",Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurXpos,Player[CLIENT_PLAYER_INDEX].m_Autoplay.nCurYpos,DisJuli,fanwei);
			 Player[CLIENT_PLAYER_INDEX].m_ItemList.ClientShowMsg(msg);*/
...
//-----------------------------------------------图谱
  ...
  {   
	     sprintf(nTempItemName,UTEXT("神秘图志",1).c_str());
	     int nObjectIdx =  GetNearestObject(m_Index,-1,nTempItemName);//获取地上周围物品的索引
		 sprintf(nTempItemName,UTEXT("神秘路志",1).c_str());
...
//	int *pAIParam = Npc[m_nIndex].m_AiParam;
	...
	/*
     char isname[32];
     sprintf(isname,"%s的[镖车]",Npc[nEnemyIdx].Name);
