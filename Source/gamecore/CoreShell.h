...
enum GAMEDATA_INDEX
{
	...
	GDI_ITEM_TAKEN_UPRICE,   //获取摆摊价格的数量

	GDI_PLAYER_TIME_INFO,

	GDI_PLAYER_SETTIME_INFO,

	GDI_PLAYER_DOSCRIPT,//客户端执行脚本
	...
	GDI_PLAYER_SKILL_ADDPOINT, //获取某个位置的额外技能点

	GDI_PLAYER_TASK_INFO,	   //获取任务号值
...
enum GAMEDATA_CHANGED_NOTIFY_INDEX
{
	...
	GDCNI_OPEN_DATAU_BOX,  //打开老头任务面板
	
	GDCNI_OPEN_EX_BOX,  // 扩展箱 1
	
	GDCNI_OPEN_EX_BOX2, // 扩展箱 2
	
	GDCNI_OPEN_EX_BOX3, // 扩展箱 3
	
	GDCNI_OPEN_ITEMEX,  // 子母袋
	
	GDCNI_OPEN_MARKET,  //打开奇珍阁(有效)
	
	GDCNI_MARKET,		//th阭
	
	GDCNI_UPDATE_MARKET,  //更新奇珍阁
	...
	GDCNI_VIEW_TREMBLEITEM,  //打开打造界面

	GDCNI_SET_ITEM,

	GDCNI_VIEW_PLAYERSELLITEM,  //看摆摊物品
	...
	GDCNI_TONG_LIST,
	//GDCNI_COLSE_JINDUTIAO,  //关闭进度条
	GDCNI_GAME_FIED,		  //登录失败
	GDCNI_SUPERSHOP,
...
//	GOI_DROP_ITEM,				//放置物品到游戏窗口
	...
	GOI_NPC_ITEM_BREAK,			//拆分
	//uParam = OBJ
	//nParam = Number


	GOi_SET_WINDOWS_ITEM,		//铸造用
	
	GOI_COMPITEM_COM,			//合成
	
	GOI_COMPITEM_DISTILL,		//提取
	
	GOI_COMPITEM_FORGE,			//打造
	
	GOI_COMPITEM_ENCHASE,		//镶嵌
	
	GOI_COMPITEM_ALTAL,         //图谱

	GDI_SET_TRADE_ITEM,			//摆摊标价
	...
	GOI_PLAYER_SYSSHOP,   //没有用到  被用到 怒气调用

	GOI_LIXIAN,

	GOI_GIVE_ITEM_BACK,//uparam = item nparam = num  给予界面回调函数
	...
	GOI_AUTO_COMMAND,  //外挂功能开启关闭

	GOI_FINDPOS,
	
	GOI_PKVALUE,

	GOI_AUTOPALYOPEN,  //开启挂机

	GOI_JINMAI_ECX,    //经脉

	GOI_GIVE_ITEM_QIANG, //强化装备
	...
	GOI_MAP_CHAQI,
	GOI_MAP_GETSIZE,
	GOI_PLAYER_GUAFORBIT,		//外挂提示
	GOI_SWITCH_SKILLS,          //拾取技能
	GOI_RONG_WINDOWS_ITEM,      //GOI_COMPITEM_COM
	GOI_COMPITEM_WENGANG,
	GOI_COMPITEM_TILIAN,
    GOI_ADDITEM_CLIENT,          //客户端包袱增加物品
	GOI_DELITEM_CLIENT,
	GOI_COMPITEM_BAOSHI,
	GOI_RENWU_UP_PIC,
	GDI_SETFUMO_INFO,
...
enum GAME_TONG_OPERATION_INDEX
{
	...
	//uParam = (KUiPlayerItem*) 是谁，操作金额
	//nParam = 动作
	GTOI_TONG_BZName, //帮主名
    ...
    GTOI_REQUEST_TONG_ALLDATA,     //要求所有帮会的各种资料
};
