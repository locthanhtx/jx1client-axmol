...
{
	   //CocosDenshion::AudioEngine::sharedEngine()->playBackgroundMusic("music/Am0.wav",true);//��������
	   //������
	   //CocosDenshion::AudioEngine::sharedEngine()->playEffect("music/Am0.wav");//ר���������ż�̵���Ч
	   if (!Layer::init())
	   {
		  return false;
	   }
...
//		m_MobileKey   = getappstrInfo();
		...
		// �����Դ������·��������·���������������µ���Դ	Init
		//std::vector
		/*std::vector<std::string> searchPaths = ax::FileUtils::getInstance()->getSearchPaths();
...
//		size               = ax::Director::getInstance()->getWinSize();
    ...
    {//���ǵ���
        ...
        if (dwmask.Load(nTempstr))
        {
            ...
            sprintf(nTempPaht,"http://%s/",donwPath);
...
//        if (sprite)//�����߳�
    ...
    {//����һ������Ի��� д��ip
        ...
        if (dwmask.Load(nTempstr))
        {
            ...
            {
                Layer * sAccLayer = KuiInPutIp::create(this,CC_CALLBACK_1(KUpdateLayer::startInPutCallback));
...
void KUpdateLayer::update(float delta)
{
	if  (_clientNoNetWork && !isOver) return;
	 ...
	 {
		sprintf(tmp,"%%%d,%s:%u",(int)(((float)m_nLoadedResources/m_nAllLoadResources)*100),nCurpFileName.c_str(),writelen);
	 }
	 else
...
//	_loadingThread = new std::thread(&KUpdateLayer::loadImage, this);
	...
	/*ax::Director::getInstance()->getTextureCache()->addImageAsync("transence.png", this, callfuncO_selector(KTranSence::loadingCallBack));
	ax::Director::getInstance()->getTextureCache()->addImageAsync("transence.png", this, callfuncO_selector(KTranSence::loadingCallBack));
	ax::Director::getInstance()->getTextureCache()->addImageAsync("transence.png", this, callfuncO_selector(KTranSence::loadingCallBack));
	ax::Director::getInstance()->getTextureCache()->addImageAsync("transence.png", this, callfuncO_selector(KTranSence::loadingCallBack));
	ax::Director::getInstance()->getTextureCache()->addImageAsync("transence.png", this, callfuncO_selector(KTranSence::loadingCallBack));
