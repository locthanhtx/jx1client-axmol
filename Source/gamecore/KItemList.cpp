...
//以下结构发向界面更新物品
	...
	if (m_Items[nFIdx].nPlace != pos_trade1)
		CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&pInfo, 0);  //通知客户端改变更新
	else
		CoreDataChanged(GDCNI_TRADE_DESIRE_ITEM, (uintptr_t)&pInfo, 0);
...
void KItemList::AutoEquip(ItemPos SrcPos,ItemPos DesPos,DWORD nSrcItemDwIdx,int nEuqPlace,int nKind)
{
	...
	int PartConvert[itempart_num] =
	{
		...
		//开始交换物品
	       //设置减少
		  if (nKind<=0) //换装
		    CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&pInfo1, 0); //容器减少

		   CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&pInfo2, 1); //容器增加
}
...
void KItemList::SetClient(int nMon1, int nMon2, int nMon3)
{
	...
	//CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&sMoney, 1);
}
...
//==================多容器检测结束======================================
	...
	//nParam = bAdd -> 0值表示减少这个物品，非0值表示增加这个物品
		...
		//开始交换物品
	       //设置减少
		   CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&pInfo1, 0);//容器减少
		   CoreDataChanged(GDCNI_HOLD_OBJECT, (uintptr_t)&pInfo2, 0);   //手里的东西减少
           //设置增加
		   CoreDataChanged(GDCNI_HOLD_OBJECT, (uintptr_t)&pInfo1, 1);   //手里增加
		   CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&pInfo2, 1);//容器增加
	}
...
BOOL	KItemList::AutoMoveItem(ItemPos SrcPos,ItemPos DesPos,unsigned int nItemdwID)
{
	...
	CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&sSrcInfo,0);  //源容器消除图标
	CoreDataChanged(GDCNI_OBJECT_CHANGED, (uintptr_t)&sDestInfo,1); //目标容器增加图标
