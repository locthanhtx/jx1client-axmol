...
static int32_t posrelat (int32_t pos, size_t len) {
  /* relative string position: negative means back from end */
  return (pos>=0) ? pos : (int32_t)len+pos+1;
}
...
static int str_sub (lua_State *L) {
  ...
  int32_t start = posrelat(luaL_check_long(L, 2), l);
  int32_t end = posrelat(luaL_opt_long(L, 3, -1), l);
  if (start < 1) start = 1;
  if (end > (int32_t)l) end = l;
  if (start <= end)
...
static int str_byte (lua_State *L) {
  size_t l;
  const char *s = luaL_check_lstr(L, 1, &l);
  int32_t pos = posrelat(luaL_opt_long(L, 2, 1), l);
...
struct Capture {
  ...
  struct {
    const char *init;
    int32_t len;  /* -1 signals unfinished capture */
...
static const char *max_expand (lua_State *L, const char *s, const char *p,
  ...
  int32_t i = 0;  /* counts maximum expand for item */
...
static int str_find (lua_State *L) {
  ...
  int32_t init = posrelat(luaL_opt_long(L, 3, 1), l1) - 1;
  struct Capture cap;
  ...
  while (*strfrmt) {
    if (*strfrmt != '%')
      ...
      if (cap.capture[0].len > 2 || cap.capture[1].len > 2 ||  /* < 100? */
        ...
        lua_error(L, "invalid format (width or precision too int32_t)");
      ...
      switch (*strfrmt++) {
        ...
        case 'o':  case 'u':  case 'x':  case 'X':
          sprintf(buff, form, (size_t)luaL_check_number(L, arg));
          break;
        ...
        case 's': {
          ...
          if (cap.capture[1].len == 0 && l >= 100) {
            /* no precision and string is too int32_t to be formatted;
