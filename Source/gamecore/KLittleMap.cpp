...
void	KLittleMap::Init(int nLeft, int nTop, int nRight, int nBottom)
{
	...
	this->m_nMapWidth		= nRight - nLeft + 1;	// dat �ļ�
	this->m_nMapHeight		= nBottom - nTop + 1;   //�ļ���
...
//		g_SetFilePath("\\");
		if (cColorIni.Load(defLITTLE_MAP_SET_FILE))
		{
			...
			int nCurVal=0;
			cColorIni.GetInteger("MapColor", "TeammateShow", 1, &nCurVal);
			this->m_bTeammateShowFlag = (nCurVal>0)?true:false;
			...
			cColorIni.GetInteger("MapColor", "PlayerShow", 1, &nCurVal);
			this->m_bPlayerShowFlag = (nCurVal>0)?true:false;
			...
			cColorIni.GetInteger("MapColor", "FightNpcShow", 1, &nCurVal);
			this->m_bFightNpcShowFlag = (nCurVal>0)?true:false;
			...
			cColorIni.GetInteger("MapColor", "NormalNpcShow", 1, &nCurVal);
			this->m_bNormalNpcShowFlag = (nCurVal>0)?true:false;
